...
-- 1. 테이블 생성(제약조건)
-- INSERT 구문
-- INSERT INTO TABEL_NAME([컬럼리스트]) VALUES(VALUE, VALUE) ;
-- 2. 제약조건(NOT NULL - 컬럼 레벨 제약만 가능, UNIQUE, PRIMARY KEY, FOREIGEN KEY, CHECK)
-- 2-1. 제약조건 설정방법 : 테이블 레벨, 컬럼 레벨
...
-- 데이터의 무결성 : PRIMARY KEY(ID)
INSERT INTO TEST_MEMBER(ID, PWD, ADDR) VALUES('EUN','JEONG','INCHEON')
INSERT INTO TEST_MEMBER VALUES('eun','EUN',NULL) 
INSERT INTO TEST_MEMBER(ID,PWD) VALUES('ADMIN','EUN') 

SELECT *
FROM TEST_MEMBER ;

-- FK (부모에 의존하는 데이터이거나 NULL 값 허용)
DROP TABLE TEST_FK ;
-- 외래키 설정방법 첫번째
...
INSERT INTO TEST_FK VALUES('B','EUN','U1')
...
INSERT INTO TEST_COMPOSIT_PK VALUES('JEONG','EUN',100,'F')
...
INSERT INTO TEST_COMPOSIT_FK VALUES('ABC','JEONG','EUN')
...
INSERT INTO TEST_S1 VALUES('EUN',11)
...
INSERT INTO TEST_S2 VALUES('10','EUN')
INSERT INTO TEST_S2(TID) VALUES('20')
...
INSERT INTO TEST_CHILD VALUES('300','EUN',NULL)

SELECT *
FROM TEST_CHILD

-- VIEW : 읽기전용
CREATE OR REPLACE VIEW VIEW_NAME(ALAIS)
AS SUBQUERY ;
    
CREATE OR REPLACE VIEW V_EMP(N, D)
...
-- 직급이 '사원'인 직원의 이름, 부서이름, 직급을 갖는 V_EMP_DEPT_JOB 뷰 작성
...
-- NEXTVAL(값을 꺼낼 때, 기본키 VALUE로 주로 사용), CURRVAL(현재 값 조회)
...
-- 18번문제
-- TOP-N 분석 : 조건에 맞는 최상위 레코드 N개를 식별할 경우 사용하는 쿼리기법
-- 원리 1. 정렬2. ROWNUM 3. 부여된 순번대로 필요한 만큼 식별
-- ROWNUM 은 가상의 컬럼임
-- 부서별 평균급여보다 많은 급여를 받는 사원정보 중 사원이름, 급여조회
-- 평균급여는 정수3자리에서 반올림
-- FROM SUBQUERY(INLINE VIEW) 이용해서 작성
-- FROM 절에 있는 인라인뷰는 논리적인 뷰 / JOIN 절에 있는 테이블은 물리적인 뷰
-- 논리뷰와 물리뷰의 조인 가능
-- RANK로 바꿔보기!
...
INSERT INTO TEST_DML VALUES ('EUN','EUN','JEONG',100,'N')
INSERT INTO TEST_DML VALUES ('ADMIN','ADMIN','KYUNG',100,'Y')
INSERT INTO TEST_DML VALUES ('ABC','ABC','DEF',100,'Y')

-- 조건은 기본키를 기준으로 함
-- 조건을 주지않으면 모든 레코드값이 변경됨
UPDATE      TEST_DML
SET         MARRIAGE = 'Y'
WHERE       ID = 'EUN' ;

DELETE FROM TEST_DML
WHERE ID = 'ADMIN' OR ID = 'ABC' ;
